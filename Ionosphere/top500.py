...
""" Read xlsx file and drop rows with NaN values for Year, Efficiency or Power consumption """
exel = pd.read_excel('TOP500_202211.xlsx', index_col=None, header=0)
df = exel.iloc[:, [0, 9, 13, 18]]
df = df.dropna()
df = df.reset_index(drop=True)

""" Divide data to training and test dataset"""
X = df.iloc[:, [2, 3]]
Y = df.iloc[:, 1]
...
""" Plot with classification divided for each year from test dataset """
...
""" Create decision tree and train it with X and y """
...
""" Plot tree with each unique year from dataset """
